document.getElementById('save').addEventListener('click', () => {
  ...
  const hideUnhighlighted = document.getElementById('hideUnhighlighted').checked;
...
document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
  checkbox.addEventListener('change', () => {
    ...
    const anyHighlightEnabled = highlightArb || highlightPreArb || highlightLessThanOneYear;
...
document.addEventListener('DOMContentLoaded', () => {
  chrome.storage.sync.get([
    ...
    document.getElementById('hideUnhighlighted').checked = data.hideUnhighlighted || false;
  });
});